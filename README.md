...
| `public/earth/warming-pattern-ssp{1_26,2_45,5_85}.png` | CMIP6 `tas` multi-model mean (ESGF mirror `gs://cmip6`), built by `scripts/data/build_warming_patterns.py` | Per-SSP surface-warming ΔT pattern (2081–2100 vs 1995–2014); the globe rescales it to the assessed/observed global warming (AR6 pattern-scaling). CC BY 4.0 |
| `public/earth/nsidc-sea-ice-decades.png` | NSIDC Sea Ice Index v4 September extent polygons, built by `scripts/data/build_sea_ice_decades.py` | Observed 1980/2000/2010/2020 September extent packed in R/G/B/A; the globe morphs between them for years ≤2025 |
| `public/earth/coast-glow-2048.png` | Natural Earth 50m coastline, built by `scripts/data/build_coast_glow.py` | Coastal-zone emphasis distance field scaled by assessed GMSL — **not** an inundation or flood map |

NASA imagery is public domain under NASA's media usage guidelines; courtesy NASA Earth Observatory. The derived masks are rendering aids only and are never presented as observed scientific data. CMIP6 warming patterns and NSIDC sea-ice data carry their own licenses (CC BY 4.0 and NSIDC citation-required, respectively); see [`DATA_LICENSES.md`](DATA_LICENSES.md) for full provenance and required attribution.
